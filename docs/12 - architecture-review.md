...
| Test pyramid | ✅ (283 unit / 53 integration / 6 E2E (+ 17 opt-in DynamoDB); 342 pass / 17 skipped, 359 with DynamoDB) |

## Remaining issues

Everything still open, **ordered by the risk of making the change, highest
first**.

| Ref | Risk (of the fix) | Description | Affects | Recommendation | Effort | Deferred because |
| --- | --- | --- | --- | --- | --- | --- |
| **S1-residual** | **medium** — changes the ingest path; the in-memory repo would need a matching transactional primitive | A crash *during* the delivery-stub materialise pass (or in `findSubscriptionsForEvent` before it) still drops the not-yet-written subscribers — recovery keys on existing delivery rows. Window is milliseconds. | **Production** | Write the event + all delivery stubs in one transaction (DynamoDB `TransactWriteItems`, ≤100 items) at ingest, or an outbox table drained by a worker. | ~2–4 hrs (transaction) / ~1 day (outbox) | Disproportionate for the timebox; the common case (crash after materialise) is already covered. Commented at `dispatcher.ts:dispatchEvent`. |
| **S8** | **low** for a cap / `?limit=`; **medium** for full pagination (new abstraction, changes the response shape) | `GET /deliveries` & `GET /subscriptions` return the whole result set — no `?limit=`, no cursor. A hot `eventId`/`status` loads a whole partition into memory and one response body. | **Production** | (a) internal hard cap `MAX_LIST_RESULTS` (~1000) + `warn` on truncation; (b) add validated `?limit=` (1–500, default 100); (c) full continuation cursor — repo-agnostic opaque token (in-memory offset vs DynamoDB `LastEvaluatedKey`), base64 encode/decode, `nextCursor` in the body. | (a) ~30 min · (b) +~30 min · (c) ~2–3 hrs | At the challenge's data scale nothing lists more than tens of rows; a partial pagination API is worse than none. Commented in code. |
| **Test gap** | **low** | Nothing exercises the S1 residual window or a real process kill. | **Test only** | *Not* a subprocess crash harness (~2–3 hrs, inherently flaky — the sub-millisecond window can't be hit deterministically). Instead, fault injection: a fake `DeliveryRepository` whose `save` fails/hangs on the Nth call. Covers `dispatch.partial_failure` and "materialised-then-recovered". | ~30 min | The high-value fault-injection subset is cheap and could be added later; the subprocess harness is not worth it. |
| **File size** | **none** (maintainability only) | `dispatcher.ts` (~360 lines) and `dispatcher.test.ts` (~560 lines) are large. | **Neither** | Split `dispatcher.test.ts` into `dispatcher-delivery.test.ts` + `dispatcher-retry.test.ts`. Leave `dispatcher.ts` — the class is cohesive (fan-out + attempt + retry share `inFlight`/`scheduledRetries` state); splitting the class adds coupling. If it grows, extract a retry-scheduling collaborator. | test split ~15 min | Soft finding; not material. Noted in the `dispatcher.ts` header. |
