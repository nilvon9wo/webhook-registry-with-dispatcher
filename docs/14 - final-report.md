...
6. **The DynamoDB tests were verified against real AWS**, not DynamoDB Local. The Local / Docker path uses the same `DYNAMODB_ENDPOINT` configuration and is a standard emulator, but it was not exercised during development; the README says so plainly rather than listing Docker as a tested requirement.
